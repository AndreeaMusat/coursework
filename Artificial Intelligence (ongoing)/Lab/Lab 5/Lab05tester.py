...
# tests for equality of v (not yet evaluated) and c (evaluated).
def test(v, c, environment, idx):
    value = eval(v, environment)
    result = value == c
    return pTest(idx, result, value, c, v)

# tests for equality of v and c (both evaluated).
...
#  tuples (type, par..ams, test_value)
...
# tests if two lists are the same. The test does not support lists with duplicate elements
def testL(Lv, Lc, env, idx):
    test = Lv
    ...
    else: "a list also containing " + ", ".join([e for e in Lc if e not in Lv])[:-2]
    return pTest(idx, result, Lv, Lc, test)
...
testBatch[0] = [(test, [
            ...
            ("is_atom("+args_3+"[2])", True, \
                [("MinLenList", 3, args_3, "N/A. Get_args returns a list that is shorter than 2.")]),
            # 20
...
testBatch[3] = [(testUnify, [
            ...
            ("make_atom('P', make_var('x'))", "make_atom('P', make_var('y'))", "{}", "{'x': make_var('y')}"),
            #12 # P(y) vs P(x) -> {x: y} (or equivalent)
            ("make_atom('P', make_var('y'))", "make_atom('P', make_var('x'))", "{}", "{'x': make_var('y')}"),
        ])]
